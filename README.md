...
**[QuantumATK](2-计算/QuantumATK.md)**

**QuantumATK** 是一款集成的原子级模拟软件平台，用于研究材料的电子结构、量子输运和原子尺度性质。它结合了第一性原理计算（如密度泛函理论 DFT）与半经验方法，广泛应用于半导体器件设计、材料开发和纳米技术研究中。通过图形化界面和脚本化工作流程，用户可以高效地构建模型、运行模拟并分析结果。

**[PWmat](2-计算/PWmat.md)**

**PWmat** 是一款高性能的第一性原理计算软件，基于密度泛函理论（DFT）和平面波赝势方法，专为 GPU 架构优化，具备卓越的并行计算能力。相比传统 CPU 程序，PWmat 在大规模体系下可实现数十倍的加速，显著提升模拟效率。它广泛应用于材料科学、半导体、新能源等领域，特别适合在超算平台上进行高通量和工业级材料模拟。

---

### 免费

**[Quantum ESPRESSO](2-计算/Quantum_ESPRESSO.md)**
...
**[SIESTA](2-计算/SIESTA.md)**

**SIESTA** 是一款基于密度泛函理论（DFT）的第一性原理计算软件，采用数值原子轨道（NAO）作为基组，相比平面波方法具有内存占用小、计算效率高的优点，特别适合研究包含几百到上万原子的纳米材料、表面与界面、缺陷和分子体系，可用于计算材料的电子结构、总能、力、结构优化以及分子动力学等性质，是研究大尺度体系电子结构问题的重要工具。

**[WIEN2k](2-计算/WIEN2k.md)**

**WIEN2k** 是一套广泛应用于计算凝聚态物理与材料科学的**第一性原理电子结构计算软件包**，基于密度泛函理论（DFT）并采用全电子、全**势线性化缀加平面波（FLAPW）方法**，以高精度著称。它无需赝势、对芯态与价态一视同仁，特别适合研究包含重元素、自旋轨道耦合、磁性与强关联效应（如 DFT+U）的体系，在能带结构、态密度、电荷密度以及拓扑性质等计算中被视为精度基准工具之一。
...
使用机器学习的话，这个建议学习这个软件。该软件更新很快。对中国用户很友好（是因为程序开发团队来自中国科学技术大学）。

**[The Elk Code](2-计算/The_Elk_Code.md)**
...
**[atomkit](2-计算/atomkit.md)**

**ATOMKIT**（原子工坊） 是一款专为计算材料科学家打造的可视化结构建模与前处理平台。我们致力于解决 DFT 计算中最耗时、最易出错的“最后一公里”问题——结构构建。通过将复杂的几何变换算法封装在直观的 Web 界面后，ATOMKIT 让无论是刚入门的研究生还是资深的 PI，都能在浏览器中以“搭积木”般的方式，快速构建出原子级的精准模型。

**[Pymatgen](2-计算/pymatgen.md)**
...
**[Materials Studio 入门到精通](2-计算/Materials_Studio.md)**
...
**[Vampire](2-计算/Vampire.md)**

**Vampire** 是一款专用于磁性材料模拟的开源软件，基于原子尺度的自旋动力学方法，能够精确描述磁性系统在不同温度和时间尺度下的行为。它支持包括铁磁、反铁磁、交换偏置等多种磁性现象的建模，适用于研究磁畴演化、热扰动、自旋转移力矩等效应。Vampire 具备良好的并行性能，适合大规模磁性系统模拟，广泛应用于磁存储、磁性材料设计和自旋电子学等前沿研究领域。
...
**[谷歌学术镜像](https://xs.xasa.top/)**

有时候谷歌学术无法访问，可以用这个救一下急。

我发现从谷歌学术或者谷歌学术镜像导入的文献引用数据是有一些小问题的。在使用 Latex 这类工具进行论文写作的时候，需要我们逐个文献进行人工校对来防止引用文献格式的错误。